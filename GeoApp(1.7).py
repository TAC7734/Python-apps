...
# --- Utility Functions for App Launcher (MODIFIED) ---
...
def clean_file_path_logic(relative_path):
    ...
    # Added .url and .website to the list that needs to be displayed fully
...
def load_launcher_apps(script_dir):
    ...
    using their cleaned names as keys, including the full directory path for display.
    """
    launcher_path = os.path.join(script_dir, LAUNCHER_FOLDER_NAME)
    # New structure: {cleaned_name: {'path': full_path, 'folder_structure': formatted_directory_path}}
    app_data = {} 
    
    if not os.path.exists(launcher_path):
        return app_data, f"Error: '{LAUNCHER_FOLDER_NAME}' folder not found at {launcher_path}"

    # Search patterns - ADDED *.url and *.website
    ...
    for pattern in search_patterns:
        ...
        for full_path in glob(os.path.join(launcher_path, '**', pattern), recursive=True):
            ...
            # Use the new path cleaning logic to get the clean name
            cleaned_name = clean_file_path_logic(relative_path)
            
            # Get the absolute directory path
            full_directory_path = os.path.dirname(full_path)
            
            # Format the directory path to use backslashes and end with a slash, 
            # for a consistent absolute path display like C:\example\
...
# --- Measurement Conversion Constants (MODIFIED to include Liquid and Temperature) ---

# Length: All conversions are based on a central unit (meter)
# Format: {unit_name: conversion_factor_to_base_unit}
LENGTH_CONVERSIONS = {
    "Meter (m)": 1.0,
    "Kilometer (km)": 1000.0,
    "Centimeter (cm)": 0.01,
...
# Liquid Volume: All conversions are based on a central unit (Liter)
LIQUID_CONVERSIONS = {
    "Liter (L)": 1.0,
    "Milliliter (mL)": 0.001,
    "Cubic Meter (m³)": 1000.0,
    "US Gallon (gal)": 3.78541,
    "Imperial Gallon (gal)": 4.54609,
    "US Quart (qt)": 0.946353,
    "US Pint (pt)": 0.473176
}

# Temperature: Units listed only for selection, conversion requires formulas
TEMPERATURE_UNITS = ["Celsius (°C)", "Fahrenheit (°F)", "Kelvin (K)"]

# Main mapping for the conversion selector
CONVERSION_MAP = {
...
class SystemUtilityApp:
    def __init__(self, master):
        ...
        # Data storage
        self.adapter_data = {}
        # Stores all found applications: {cleaned_name: {'path': full_path, 'folder_structure': formatted_directory_path}}
        self.app_data = {} 
        ...
        # Measurement Converter Variables (NEW/MODIFIED)
        self.conversion_type_var = tk.StringVar(value=list(CONVERSION_MAP.keys())[0]) # Default to Length
        # OptionMenu widgets that need to be dynamically updated
        ...
        self.measurement_tab = tk.Frame(self.notebook, bg=BACKGROUND_COLOR) 
        ...
        self.notebook.add(self.measurement_tab, text='Measurement Converter') 
        ...
        self.setup_measurement_converter_section(self.measurement_tab) # NEW SETUP CALL
    ...
    # --- Launcher Methods (UPDATED with Copy Button) ---
    ...
    def setup_launcher_section(self, parent_frame):
        ...
        # Row 1: Search Entry Field and Launch Button
        self.search_app_var = tk.StringVar(launcher_frame, value="") # Start empty
        ...
        # Row 3: Folder Structure Label 
        tk.Label(launcher_frame, text="Full Directory Path:", font=('Segoe UI', 10, 'bold'), 
                 bg=CARD_COLOR, fg="#000000", anchor="w").grid(row=3, column=0, sticky="w", padx=5, pady=5)
                 
        # Row 4: Folder Structure Value and Copy Button
        self.folder_structure_var = tk.StringVar(value="Select an application to see its path.")
        ...
        # Label now only spans column 0
        self.folder_structure_label.grid(row=4, column=0, sticky="ew", padx=5, pady=2) 
        
        # NEW: Copy Button for Folder Structure
        tk.Button(launcher_frame, text="Copy", command=self.copy_folder_structure,
                  font=self.font_normal_small, bg=SECONDARY_COLOR, fg="white", bd=0, 
                  padx=5, pady=1, relief=tk.GROOVE, 
                  activebackground="#4db850", activeforeground="white").grid(row=4, column=1, sticky="w", padx=5)

        launcher_frame.grid_columnconfigure(0, weight=1)
        launcher_frame.grid_columnconfigure(1, weight=0)
    ...
    def update_app_launcher_dropdown(self):
        ...
        self.search_app_var.set("") # Ensure search field is empty on load
        self.folder_structure_var.set("Select an application to see its path.")
        ...
        if app_names:
            self.update_app_suggestions() # Initial population of the listbox with all names
            self.status_var.set(f"Loaded {len(app_names)} files. Start typing to search.")
        else:
            self.suggestion_listbox.delete(0, tk.END)
            self.suggestion_listbox.insert(tk.END, f"No files found in '{LAUNCHER_FOLDER_NAME}'...")
    ...
    def update_app_suggestions(self, event=None):
        ...
        if not search_term:
            filtered_apps = all_cleaned_names
            if not filtered_apps:
                self.suggestion_listbox.insert(tk.END, "No files available...")
                self.folder_structure_var.set("Select an application to see its path.")
                return
        else:
            filtered_apps = [name for name in all_cleaned_names if search_term in name.lower()]

        # Populate the listbox with filtered results
        if filtered_apps:
            for name in filtered_apps:
                self.suggestion_listbox.insert(tk.END, name)
            # Auto-select the first item if the list is filtered and the user is typing
            if search_term and len(filtered_apps) > 0:
                self.suggestion_listbox.select_set(0)
                # Manually call update display. The core change is inside this function now.
                self.select_app_from_list(None) 
        else:
    ...
    def select_app_from_list(self, event):
        """
        Updates the Folder Structure display based on listbox selection.
        The search input field is intentionally NOT updated here.
        """
        ...
        if selection:
            ...
            if selected_cleaned_name in self.app_data:
                ...
                # Update the Folder Structure Display Box
                self.folder_structure_var.set(app_details['folder_structure'])
                self.status_var.set(f"Selected: {selected_cleaned_name}")

            else:
                self.folder_structure_var.set("Invalid selection.")
        else:
            # Handle case where selection is cleared or invalid
            pass

    def copy_folder_structure(self):
        """Copies the currently displayed Full Directory Path to the system clipboard."""
        path = self.folder_structure_var.get()
        
        if path.startswith("Select an application") or path.startswith("..."):
            self.status_var.set("Copy failed: No valid path available.")
            return
        ...
        try:
            ...
            self.master.update() 
            self.status_var.set(f"Copied folder path: {path} to clipboard.")
            messagebox.showinfo("Copied", f"Full Directory Path copied to clipboard:\n{path}")
        except Exception as e:
            self.status_var.set(f"Copy error: {e}")
    ...
    def launch_application(self):
        """
        Launches the application based on the currently selected item in the listbox.
        If nothing is selected, it attempts to launch the name typed in the input box.
        """
        selection = self.suggestion_listbox.curselection()
        
        # 1. Determine which app name to use (prioritize listbox selection)
        if selection:
            cleaned_app_name = self.suggestion_listbox.get(selection[0])
        else:
            # 2. Fallback: Use the text currently typed by the user (requires an exact match)
            cleaned_app_name = self.search_app_var.get().strip()
            
        if not cleaned_app_name:
            messagebox.showwarning("Warning", "Please select an application from the list or type a name.")
            self.status_var.set("Launch failed: No application specified.")
            return
            
        if cleaned_app_name in self.app_data:
            path = self.app_data[cleaned_app_name]['path']
        else:
            messagebox.showerror("Error", f"File '{cleaned_app_name}' not found. Please select a suggestion or type the exact name.")
    ...
    # --- Measurement Converter Methods (MODIFIED) ---

    def setup_measurement_converter_section(self, parent_frame):
        """Builds the GUI elements for the measurement converter, now supporting multiple types."""
        measure_frame = tk.LabelFrame(parent_frame, text="Unit Converter", 
                                  font=self.font_large, bg=CARD_COLOR, fg=TEXT_COLOR,
                                  padx=15, pady=15, bd=1, relief=tk.RIDGE)
        measure_frame.pack(pady=15, fill="x", padx=10)

        # Row 0: Conversion Type Selector
        tk.Label(measure_frame, text="Conversion Type:", font=('Segoe UI', 10, 'bold'), 
                 bg=CARD_COLOR, fg=TEXT_COLOR).grid(row=0, column=0, sticky="w", padx=5, pady=5)

        conversion_types = list(CONVERSION_MAP.keys())
        # The trace must be added BEFORE the OptionMenu is created to ensure the command is available
        ...
        tk.Label(measure_frame, text="Input Value:", font=self.font_normal, 
                 bg=CARD_COLOR, fg=TEXT_COLOR).grid(row=1, column=0, sticky="w", padx=5, pady=5)
        ...
        # Input Unit Selector Placeholder
        ...
        # Output Unit Selector Placeholder
        ...
        tk.Label(measure_frame, text="Output Value:", font=self.font_normal, 
                 bg=CARD_COLOR, fg=TEXT_COLOR).grid(row=3, column=0, sticky="w", padx=5, pady=5)
        ...
        measure_frame.grid_columnconfigure(3, weight=0) # Button column
        
        # Initial population of the unit dropdowns
        self.update_units_dropdowns()


    def update_units_dropdowns(self, *args):
        """Dynamically updates the input and output unit dropdowns based on conversion type."""
        conversion_type = self.conversion_type_var.get()
        unit_data = CONVERSION_MAP.get(conversion_type, {})
        
        # For non-temperature conversions, unit_data is a dict keys, otherwise it's a list.
        unit_names = list(unit_data.keys()) if isinstance(unit_data, dict) else unit_data

        # Clear existing menus
        self.input_unit_menu["menu"].delete(0, "end")
        self.output_unit_menu["menu"].delete(0, "end")
        
        # Set new default values
        default_unit = unit_names[0] if unit_names else "N/A"
        ...
        if unit_names:
            for name in unit_names:
                self.input_unit_menu["menu"].add_command(label=name, command=tk._setit(self.input_unit_var, name))
                self.output_unit_menu["menu"].add_command(label=name, command=tk._setit(self.output_unit_var, name))
        else:
            self.input_unit_menu["menu"].add_command(label="No Units", command=lambda: None)
            self.output_unit_menu["menu"].add_command(label="No Units", command=lambda: None)
            
        # Update labels for clarity (e.g., Temperature doesn't use 'factors')
        if conversion_type == "Temperature":
             self.input_unit_label.config(text="From Unit:")
             self.output_unit_label.config(text="To Unit:")
        else:
             self.input_unit_label.config(text="Input Unit:")
             self.output_unit_label.config(text="Output Unit:")

        self.measure_result_value_var.set("Ready for conversion.")
    ...
    def convert_measurement(self):
        ...
        try:
            ...
            if conversion_type in ["Length", "Liquid Volume"]:
                ...
                # 2. Convert from the base unit to the output unit
                output_factor = unit_map[output_unit_name]
                converted_val = base_value / output_factor

            elif conversion_type == "Temperature":
                # Handle Temperature conversion using formulas (base unit is Celsius)
                # Step 1: Convert input to Celsius (C)
                ...
                # Step 2: Convert from Celsius (C) to the output unit
    ...
    def copy_converted_measurement(self):
        """Copies the currently displayed converted measurement value to the system clipboard."""
        output = self.measure_result_value_var.get()
        
        if "Error" in output or output.startswith("Select type") or output.startswith("Ready for"):
            self.status_var.set("Copy failed: No valid measurement output to copy.")
            return
        ...
        try:
            ...
            self.master.update() 
            self.status_var.set(f"Copied output: {output} to clipboard.")
            messagebox.showinfo("Copied", f"Converted measurement value copied to clipboard:\n{output}")
        except Exception as e:
            self.status_var.set(f"Copy error: {e}")
            messagebox.showerror("Copy Error", f"Failed to copy output: {e}")
