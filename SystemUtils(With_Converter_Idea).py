...
from tkinter import messagebox, ttk # Added ttk for the Notebook widget
import subprocess
import re
import platform
import os
...
def get_network_info():
    ...
    output, error = run_command("ipconfig /all")
    ...
    # Initialize a dummy adapter entry for the start of the loop
    all_adapters[current_adapter_name] = {"ipv4": "Not Found", "mac": "Not Found"}

    for line in lines:
        line = line.strip()

        # 1. Look for a new adapter block
        adapter_match = adapter_name_pattern.search(line)
        if adapter_match:
            current_adapter_name = adapter_match.group(1).strip()
            # Initialize new adapter entry
            all_adapters[current_adapter_name] = {"ipv4": "Not Found", "mac": "Not Found"}
            continue # Move to the next line to parse details

        # 2. Parse details within the current adapter block
        if current_adapter_name in all_adapters:
            ...
            if ipv4_match and data["ipv4"] == "Not Found":
                 ...
                 # Filter out standard loopback/unassigned addresses
            ...
            if mac_match and data["mac"] == "Not Found":
                # Normalize the MAC address format to use hyphens
                data["mac"] = mac_match.group(1).replace(':', '-')

    # Filter out the initial "N/A" dummy entry and any adapters without an IP or MAC (inactive/disconnected)
    filtered_adapters = {name: data for name, data in all_adapters.items() 
                         if data.get("ipv4") != "Not Found" or data.get("mac") != "Not Found"}
...
def run_command(command):
    ...
    try:
        ...
        if process.returncode != 0:
            return None, f"Command failed with error: {stderr.strip()}"
...
def load_launcher_apps(script_dir):
    """Scans the designated folder for executable files."""
    ...
    # Search for common executable/shortcut files
    search_patterns = ['*.exe', '*.lnk', '*.bat', '*.com', '*.cmd']
    
    for pattern in search_patterns:
        # Use recursive=False, we only scan the top level
        for full_path in glob(os.path.join(launcher_path, pattern)):
            filename = os.path.basename(full_path)
            # Remove extension for a friendly name, unless it's a desktop shortcut
...
class SystemUtilityApp:
    def __init__(self, master):
        self.master = master
        master.title("Windows System & App Utility")
        ...
        self.script_dir = os.path.dirname(os.path.abspath(__file__)) if '__file__' in locals() else os.getcwd()
        ...
        # Use fill='both', expand=True to make the notebook responsive
        ...
        # --- Setup Sections using the new tab frames ---
    ...
    def update_network_data(self):
        ...
        if adapter_names:
            ...
            self.selected_adapter.set("No Adapters Found")
    ...
    def setup_launcher_section(self, parent_frame):
        """Builds the GUI elements for the application launcher."""
        launcher_frame = tk.LabelFrame(parent_frame, text="Program Launcher (from folder)", 
                                       font=self.font_large, bg=CARD_COLOR, fg=TEXT_COLOR,
                                       padx=15, pady=15, bd=1, relief=tk.RIDGE)
        launcher_frame.pack(pady=15, fill="x", padx=10)
        
        # Dropdown Label
        tk.Label(launcher_frame, text="Select Application:", font=self.font_normal, 
                 bg=CARD_COLOR, fg=TEXT_COLOR).grid(row=0, column=0, sticky="w", padx=5, pady=5)
        
        # Dropdown Variable
        self.selected_app = tk.StringVar(launcher_frame)
        # Placeholder options - will be populated on load
        self.app_options = tk.OptionMenu(launcher_frame, self.selected_app, 'Scanning...')
        self.app_options.config(font=self.font_normal, bg="#E0E0E0", activebackground="#D0D0D0", relief=tk.FLAT)
        self.app_options["menu"].config(font=self.font_normal)
        self.app_options.grid(row=1, column=0, sticky="ew", padx=5, pady=5)
        
        # Launch Button
        tk.Button(launcher_frame, text="Launch App", command=self.launch_application,
                  font=self.font_normal, bg=PRIMARY_COLOR, fg="white", bd=0, padx=10, pady=5,
                  relief=tk.GROOVE, # Smoother button look
                  activebackground="#0056b3", activeforeground="white").grid(row=1, column=1, sticky="e", padx=5, pady=5)
        
        launcher_frame.grid_columnconfigure(0, weight=1)

    def update_app_launcher_dropdown(self):
        """Scans the designated folder and populates the app dropdown."""
        app_list, error = load_launcher_apps(self.script_dir)
        self.app_paths = app_list
        app_names = list(self.app_paths.keys())
        
        menu = self.app_options["menu"]
        menu.delete(0, "end")
        
        if error:
            menu.add_command(label=error, command=lambda: None)
            self.selected_app.set(f"Setup Error: {LAUNCHER_FOLDER_NAME} folder missing.")
            self.status_var.set(error)
            return

        if app_names:
            for name in app_names:
                menu.add_command(label=name, command=lambda value=name: self.selected_app.set(value))
            
            self.selected_app.set(app_names[0])
            self.status_var.set(f"Launcher loaded with {len(app_names)} applications.")
        else:
            self.selected_app.set(f"No Apps Found in '{LAUNCHER_FOLDER_NAME}'")
            menu.add_command(label="No Apps Found", command=lambda: None)
            self.status_var.set(f"No executables found in the '{LAUNCHER_FOLDER_NAME}' folder.")

    def launch_application(self):
        """Launches the application selected in the dropdown."""
        app_name = self.selected_app.get()

        if app_name not in self.app_paths:
            messagebox.showerror("Error", f"Application '{app_name}' not found or path is invalid.")
    ...
    def setup_converter_section(self, parent_frame):
        ...
        converter_frame = tk.LabelFrame(parent_frame, text="Number Base Converter", 
                                  font=self.font_large, bg=CARD_COLOR, fg=TEXT_COLOR,
                                  padx=15, pady=15, bd=1, relief=tk.RIDGE)
        converter_frame.pack(pady=15, fill="x", padx=10)

        # Row 0: Labels
        tk.Label(converter_frame, text="Input Value (Decimal, 0b..., 0x...):", font=self.font_normal, 
                 bg=CARD_COLOR, fg=TEXT_COLOR).grid(row=0, column=0, sticky="w", padx=5, pady=5)
        tk.Label(converter_frame, text="Output Base (2-20):", font=self.font_normal, 
        ...
        # Base Selector Dropdown (2 to 20)
        self.output_base_var = tk.StringVar(converter_frame, value="10") # Default to Decimal
        base_options = [str(i) for i in range(2, 21)]
        
        base_menu = tk.OptionMenu(converter_frame, self.output_base_var, *base_options)
        base_menu.config(font=self.font_normal, bg="#E0E0E0", activebackground="#D0D0D0", relief=tk.FLAT)
        base_menu["menu"].config(font=self.font_normal)
        base_menu.grid(row=1, column=1, sticky="ew", padx=5)
        
        # Conversion Button
        tk.Button(converter_frame, text="Convert", command=self.convert_number,
                  font=self.font_normal, bg=PRIMARY_COLOR, fg="white", bd=0, padx=10, pady=5,
                  relief=tk.GROOVE, activebackground="#0056b3", activeforeground="white").grid(row=1, column=2, sticky="e", padx=5)

        # Row 2 & 3: Result
        tk.Label(converter_frame, text="Result:", font=self.font_normal, 
                 bg=CARD_COLOR, fg=TEXT_COLOR).grid(row=2, column=0, sticky="w", padx=5, pady=5)
                 
        self.result_value_var = tk.StringVar(value="Result will appear here.")
        tk.Label(converter_frame, textvariable=self.result_value_var, font=('Consolas', 10, 'bold'), 
                 bg="#DEDEDE", fg="#000000", anchor="w", padx=5, pady=5, relief=tk.SUNKEN).grid(row=3, column=0, sticky="ew", padx=5, columnspan=3)

        converter_frame.grid_columnconfigure(0, weight=1)
        converter_frame.grid_columnconfigure(1, weight=1)
        converter_frame.grid_columnconfigure(2, weight=0)

    def convert_number(self):
        """Converts the input number (automatically detecting binary, hex, or decimal) to the selected output base (2-20)."""
        ...
        try:
            ...
            # Helper function for custom base conversion (up to base 20)
            def dec_to_base(number, base):
                if number == 0:
                    return "0"
                if base > 20 or base < 2:
                    # Should not happen due to OptionMenu range (2-20)
                    raise ValueError("Base must be between 2 and 20")
                
                # Digits for bases up to 20: 0-9, A-J (10-19)
            ...
            # Step 1: Detect Input Base and Convert to Decimal (Base 10)
            input_base = 10 # Default base
            
            if input_str.lower().startswith('0x'):
                input_base = 16
                decimal_val = int(input_str, 16)
            elif input_str.lower().startswith('0b'):
                input_base = 2
                decimal_val = int(input_str, 2)
            else:
                # Attempt to parse as Decimal (Base 10)
                decimal_val = int(input_str, 10)
            
            # Step 2: Convert Decimal Value to Target Base
            converted_val = dec_to_base(decimal_val, output_base)
            ...
            final_result = f"Input (Base {input_base}) -> Output (Base {output_base}): {output_prefix}{converted_val}"
            
            self.result_value_var.set(final_result)
            self.status_var.set(f"Conversion successful: {decimal_val} -> {output_base}")

        except ValueError as ve:
            # Catch errors from int() conversion (e.g., non-numeric input, or letters in binary/decimal)
            error_msg = f"Error: Invalid input format or character. Details: {ve}"
...
def minimize_console_window():
    ...
    if platform.system() == "Windows":
        try:
            ...
            # Fail silently if ctypes/Windows API call fails
            pass

if __name__ == "__main__":
    # Minimize the console window immediately upon execution
    minimize_console_window()
    
    if platform.system() != "Windows":
        messagebox.showwarning("OS Warning", "This application is heavily reliant on Windows-specific commands ('ipconfig', 'os.startfile') and may not function correctly on this operating system.")
