...
class SystemUtilityApp:
    def __init__(self, master):
        ...
        # Base Converter mapping for 2, 8, 10, 16
        self.base_map = {"Binary (2)": 2, "Octal (8)": 8, "Decimal (10)": 10, "Hex (16)": 16}
    ...
    def setup_converter_section(self, parent_frame):
        """Builds the GUI elements for the number base converter with explicit input/output bases."""
        ...
        tk.Label(converter_frame, text="Result:", font=self.font_normal, 
                 bg=CARD_COLOR, fg=TEXT_COLOR).grid(row=2, column=0, sticky="w", padx=5, pady=5, columnspan=4)
        ...
        tk.Label(converter_frame, textvariable=self.result_value_var, font=('Consolas', 10, 'bold'), 
                 bg="#DEDEDE", fg="#000000", anchor="w", padx=5, pady=5, relief=tk.SUNKEN).grid(row=3, column=0, sticky="ew", padx=5, columnspan=4)
    ...
    def convert_number(self):
        ...
        try:
            # Retrieve integer values for the selected bases
            ...
            # int(string, base) handles the conversion to decimal
            ...
            if output_base == 10:
                ...
                # bin() returns '0b...'. We remove '0b'.
                converted_val = bin(decimal_val)[2:]
            elif output_base == 8:
                # oct() returns '0o...'. We remove '0o'.
                converted_val = oct(decimal_val)[2:]
            elif output_base == 16:
                # hex() returns '0x...'. We remove '0x' and convert to uppercase.
                converted_val = hex(decimal_val)[2:].upper()
            else:
                converted_val = "ERROR: Output base not supported in conversion logic (Should be 2, 8, 10, or 16)."

            # Step 3: Format the Output
            final_result = f"Input: {input_str} ({input_base_name}) \u2192 Output: {converted_val} ({output_base_name})"
            ...
            # Catch errors if the input string contains invalid digits for the selected input base
            error_msg = f"Error: Input '{input_str}' is invalid for the selected base ({input_base_name})."
