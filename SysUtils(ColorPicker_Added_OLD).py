...
def load_launcher_apps(script_dir):
    ...
    # Search for common executable/shortcut files (Windows-focused)
    ...
    for pattern in search_patterns:
        ...
        for full_path in glob(os.path.join(launcher_path, pattern)):
            ...
            # Remove extension for a friendly name
...
class SystemUtilityApp:
    def __init__(self, master):
        ...
        # CRITICAL FIX: Initialize status_var BEFORE any setup function that might call it
        ...
        # --- Setup Sections using the new tab frames (now safe) ---
        ...
        self.setup_color_picker_section(self.color_tab) # New Section Setup
        
        # Status Label: Keep outside the notebook (packed to master)
        # Note: self.status_var was defined above
    ...
    # --- Base Converter Methods (Unchanged logic, reordered) ---
    ...
    def setup_color_picker_section(self, parent_frame):
        ...
        color_frame = tk.LabelFrame(parent_frame, text="RGB to HEX Converter",
                                  font=self.font_large, bg=CARD_COLOR, fg=TEXT_COLOR,
                                  padx=15, pady=15, bd=1, relief=tk.RIDGE)
        color_frame.pack(pady=15, fill="x", padx=10)

        # Variables for R, G, B inputs and HEX output
        ...
        # --- Input Section (R, G, B) ---
        input_frame = tk.Frame(color_frame, bg=CARD_COLOR)
        input_frame.pack(fill='x', pady=10)

        # Validation command for live input checking (ensures only numbers are typed)
        validate_cmd = input_frame.register(self.validate_rgb_input)
        ...
        def create_color_input(parent, label_text, var, column):
            ...
            entry = tk.Entry(parent, textvariable=var, font=self.font_normal,
                             ...
                             validate='key', 
                             # %P is the value of the text field after the edit
                             validatecommand=(validate_cmd, '%P')) 
            entry.grid(row=0, column=column * 2 + 1, sticky="ew", padx=(0, 15))
            
            # Use trace to automatically call update_color whenever the variable changes
            var.trace_add("write", lambda *args: self.update_color())
            
            return entry

        # R, G, B fields
        self.r_entry = create_color_input(input_frame, "R (0-255):", self.r_var, 0)
        self.g_entry = create_color_input(input_frame, "G (0-255):", self.g_var, 1)
        self.b_entry = create_color_input(input_frame, "B (0-255):", self.b_var, 2)
        ...
        # --- Output Section (Color Box and HEX Value) ---
        output_frame = tk.Frame(color_frame, bg=CARD_COLOR)
        output_frame.pack(fill='x', pady=10)

        # 1. Color Display Box
        tk.Label(output_frame, text="Current Color:", font=('Segoe UI', 10, 'bold'),
                 bg=CARD_COLOR, fg=TEXT_COLOR).grid(row=0, column=0, sticky="w", padx=5, pady=5)
        ...
        # 2. HEX Output Value
        tk.Label(output_frame, text="HEX Value:", font=('Segoe UI', 10, 'bold'),
                 bg=CARD_COLOR, fg=TEXT_COLOR).grid(row=1, column=0, sticky="w", padx=5, pady=5)

        self.hex_label = tk.Label(output_frame, textvariable=self.hex_var, font=('Consolas', 10, 'bold'),
                                  bg="#DEDEDE", fg="#000000", anchor="w", padx=5, pady=5, relief=tk.SUNKEN)
        self.hex_label.grid(row=1, column=1, sticky="ew", padx=10, pady=5)
    ...
    def update_color(self):
        ...
        try:
            ...
            # Clamp values to the 0-255 range (redundant if validation is perfect, but safe)
            ...
            self.status_var.set(f"Color updated to HEX: {hex_color}")

        except Exception as e:
            # Set error state visually
            self.status_var.set(f"Color update error: {e}")
...
# --- OS Specific Functions (Minimized for other OS compatibility) ---
